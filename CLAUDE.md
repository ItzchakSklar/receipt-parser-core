...
- **OCR**: `app/services/ocr_service.py` tries Tesseract (`pytesseract`+`Pillow`) → EasyOCR → `pypdf` (text-layer PDFs), in that order. Only Tesseract+Pillow are installed by default; EasyOCR/pypdf are commented out in `requirements.txt`.
