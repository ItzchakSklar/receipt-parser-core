...
`ocr_service.py` extracts vendor name, amount, date, and tax ID and **never invents data**. An engine's result is accepted only at ≥60% average confidence with all required fields found. Every attempt (Pillow/PyMuPDF open, per-page raw OCR text, per-PSM confidence, which required field(s) failed) is logged with `print("[ocr_service] ...")` — there's no `logging.basicConfig()` anywhere in the app, so `logger.info`/`logger.debug` calls are silently swallowed by Python's root logger; `print()` is the only reliable way to get this into the console, which is why the module already used it before this comment existed.

If every engine fails or none is installed, `POST /api/invoices/upload` no longer rejects the file: it moves the saved upload into `uploads/<business_id>/unrecognized/` and creates an `Invoice` row with `is_unrecognized=True` (`Invoice.vendor_name`/`amount`/`date` get filename/zero/now placeholders purely to satisfy NOT NULL columns — they are never shown as real data behind that flag). This is the "לא מזוהים" (Unrecognized) folder: a persistent top-level entry in `InvoiceExplorer` (`UnrecognizedFolderCard`, badge = count) shown regardless of the Year/Month filter, backed by `GET /api/invoices/unrecognized`. `GET /api/invoices`, the dashboard stats query, and `POST /export-monthly` all filter `is_unrecognized == False` so these rows never leak into normal totals/folders. A user reviews one via `UnrecognizedReviewModal` (preview + form) and submits `POST /api/invoices/unrecognized/{id}/sort`, which applies the confirmed vendor/amount/date, clears the flag, and physically moves the file out of `unrecognized/` — that's the only legitimate path to `ocr_source: "manual"` for these rows. Already-saved *normal* invoices can still be corrected via `PUT/PATCH /api/invoices/{id}` (edit action in the file explorer).

`POST /api/invoices/confirm` (`InvoiceConfirmRequest`) still exists as a manual-finalize-from-`file_reference` API but nothing in the upload flow raises the `422 INVALID_OCR_DATA` it was originally built to follow — treat it as a standalone endpoint, not part of the current upload flow.
...
- `components/` — `InvoiceExplorer.tsx` (folder-style browser: `MonthFolderCard` → `ReceiptFileCard`/`ReceiptThumbnail`, with `ReceiptLightboxModal` for full-size viewing and `ContextMenu` for edit/delete/download; plus the persistent `UnrecognizedFolderCard` → `UnrecognizedFileCard` → `UnrecognizedReviewModal` for the "לא מזוהים" folder), `Dashboard.tsx` (stat cards + recharts), `UploadZone.tsx` (drag-drop; shows a `Toast` and the amber "unrecognized" banner instead of a retry-only error when OCR can't read a file), `EditInvoiceModal.tsx`, `ConflictResolutionModal.tsx`, `SendToAccountantModal.tsx`, `ConfirmDialog.tsx`/`Modal.tsx` (shared shells).
...
### CI/CD & Docker
`.github/workflows/deploy.yml` runs on every push/PR to `main`: `test-backend`/`test-frontend` jobs, then (push to `main` only) `build` pushes `backend/Dockerfile`/`frontend/Dockerfile` images to GHCR, then `deploy` SSHes into a server to `docker compose pull && up -d` the root `docker-compose.yml`. See the README's "GitHub Actions CI/CD Setup" section for required repo secrets. `backend/Dockerfile` installs `tesseract-ocr`+`tesseract-ocr-heb` via apt and copies the trained data into `$LOCALAPPDATA/SmartReceipt/tessdata` (with `LOCALAPPDATA` set to a fixed path) to match the exact directory layout `ocr_service.py` looks for — keep that layout in sync if the lookup path in `ocr_service.py` ever changes. The Dockerfile's `CMD` is shell-form (`--port ${PORT:-8000}`), not exec-form, specifically so Render's injected `$PORT` gets substituted at container start.

### Render + Vercel deployment
`app/config.py`'s `port` (bound to Render's injected `PORT` env var) and `frontend_url` (merged into `cors_origin_list` alongside `cors_origins`) exist for this path — see the README's "Deploying to Render + Vercel" section for the full click-through. `frontend/src/api/client.ts`'s axios `baseURL` reads `import.meta.env.VITE_API_BASE_URL`, needed because Vercel (frontend) and Render (backend) are on different origins, unlike the same-origin nginx-proxy setup in `docker-compose.yml`. `frontend/vercel.json` rewrites all routes to `index.html` for SPA client-side routing. **Render's free tier has an ephemeral filesystem** - `smartreceipt.db` and `uploads/` are wiped on every deploy and inactivity-triggered restart, which conflicts with this app's SQLite-file + local-disk storage model; a persistent disk (paid Render plan) or a hosted DB + object store is required for real data to survive there.
