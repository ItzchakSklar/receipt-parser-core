...
export default function ConflictResolutionModal({ conflict, onResolved, onClose }: ConflictResolutionModalProps) {
      ...
      >
        <div className="flex items-center justify-between border-b border-slate-100 px-6 py-4">
          <div className="flex items-center gap-2">
            ...
            <h3 className="text-lg font-semibold text-slate-800">אותה חשבונית נמצאה עם נתונים שונים</h3>
          </div>
        ...
        <div className="flex-1 overflow-y-auto px-6 py-5">
          <p className="mb-4 text-sm text-slate-500">
            נמצאה חשבונית קיימת מאותו ספק ועם אותו מספר חשבונית, אך הנתונים שחולצו מהקובץ החדש שונים. בחר איזו גרסה לשמור.
          </p>

          <div className="grid grid-cols-2 gap-4">
            <ReceiptPane label="קיים במערכת" isPdf={existingIsPdf} objectUrl={existingImage.objectUrl} error={existingImage.error} />
            <ReceiptPane label="הועלה כעת" isPdf={newIsPdf} objectUrl={newImage.objectUrl} error={newImage.error} />
          </div>
          ...
          <div className="mt-5">
            ...
            <DataRow label="ספק" existingValue={existing.vendor_name} newValue={newData.vendor_name} differs={existing.vendor_name !== newData.vendor_name} />
            <DataRow
              label="מס' חשבונית"
            ...
            <DataRow
              ...
              differs={new Date(existing.date).toDateString() !== new Date(newData.date).toDateString()}
            />
            <DataRow
              label="ח.פ / עוסק"
